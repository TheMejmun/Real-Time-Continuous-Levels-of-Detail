...
void Application::mainLoop() {
	while (!this->windowManager.shouldClose()) {
		...
		int move = 0;
		if (this->inputManager.getKeyState(IM_MOVE_FORWARD) == IM_DOWN_EVENT || this->inputManager.getKeyState(IM_MOVE_FORWARD) == IM_HELD) {
			move += 1;
		}
		if (this->inputManager.getKeyState(IM_MOVE_BACKWARD) == IM_DOWN_EVENT || this->inputManager.getKeyState(IM_MOVE_BACKWARD) == IM_HELD) {
			move -= 1;
		}
